...
-- RPC: edit_post
CREATE OR REPLACE FUNCTION edit_post(p_post_id UUID, p_title TEXT, p_content TEXT)
RETURNS JSONB AS $$
DECLARE
  v_post posts%ROWTYPE;
BEGIN
  IF auth.uid() IS NULL THEN RETURN jsonb_build_object('success', FALSE, 'error', 'unauthorized'); END IF;
  SELECT * INTO v_post FROM posts WHERE id = p_post_id AND author_id = auth.uid() AND is_dead = FALSE AND is_hidden = FALSE AND is_deleted = FALSE FOR UPDATE;
  IF NOT FOUND THEN RETURN jsonb_build_object('success', FALSE, 'error', 'post_not_found'); END IF;
  IF EXISTS (SELECT 1 FROM reports WHERE target_id = p_post_id AND target_type = 'post' AND status = 'pending') THEN RETURN jsonb_build_object('success', FALSE, 'error', 'pending_report_exists'); END IF;
  INSERT INTO post_edits (post_id, old_title, old_content, editor_id) VALUES (p_post_id, v_post.title, v_post.content, auth.uid());
  UPDATE posts SET title = p_title, content = p_content, edited_at = NOW() WHERE id = p_post_id;
  RETURN jsonb_build_object('success', TRUE);
END;
$$ LANGUAGE plpgsql SECURITY DEFINER SET search_path = public;
...
BEGIN
  IF auth.uid() IS NULL THEN RETURN jsonb_build_object('success', FALSE, 'error', 'unauthorized'); END IF;
  SELECT * INTO v_post FROM posts WHERE id = p_post_id AND author_id = auth.uid() AND is_deleted = FALSE FOR UPDATE;
  IF NOT FOUND THEN RETURN jsonb_build_object('success', FALSE, 'error', 'post_not_found'); END IF;
  IF EXISTS (SELECT 1 FROM reports WHERE target_id = p_post_id AND target_type = 'post' AND status = 'pending') THEN RETURN jsonb_build_object('success', FALSE, 'error', 'pending_report_exists'); END IF;
  UPDATE posts SET is_deleted = TRUE, deleted_at = NOW(), title = '[삭제된 글]', content = '[작성자에 의해 삭제되었습니다]' WHERE id = p_post_id;
  UPDATE comments SET is_deleted = TRUE, deleted_at = NOW(), content = '[삭제된 댓글]' WHERE post_id = p_post_id AND is_deleted = FALSE;
  RETURN jsonb_build_object('success', TRUE);
END;
$$ LANGUAGE plpgsql SECURITY DEFINER SET search_path = public;
...
RETURNS JSONB AS $$
DECLARE v_comment comments%ROWTYPE; v_post posts%ROWTYPE;
BEGIN
  IF auth.uid() IS NULL THEN RETURN jsonb_build_object('success', FALSE, 'error', 'unauthorized'); END IF;
  SELECT * INTO v_comment FROM comments WHERE id = p_comment_id AND author_id = auth.uid() AND is_deleted = FALSE FOR UPDATE;
  IF NOT FOUND THEN RETURN jsonb_build_object('success', FALSE, 'error', 'comment_not_found'); END IF;
  SELECT * INTO v_post FROM posts WHERE id = v_comment.post_id AND is_dead = FALSE AND is_hidden = FALSE AND is_deleted = FALSE;
  IF NOT FOUND THEN RETURN jsonb_build_object('success', FALSE, 'error', 'post_not_available'); END IF;
  INSERT INTO comment_edits (comment_id, old_content, editor_id) VALUES (p_comment_id, v_comment.content, auth.uid());
  UPDATE comments SET content = p_content, edited_at = NOW() WHERE id = p_comment_id;
  RETURN jsonb_build_object('success', TRUE);
END;
$$ LANGUAGE plpgsql SECURITY DEFINER SET search_path = public;
...
RETURNS JSONB AS $$
DECLARE v_comment comments%ROWTYPE;
BEGIN
  IF auth.uid() IS NULL THEN RETURN jsonb_build_object('success', FALSE, 'error', 'unauthorized'); END IF;
  SELECT * INTO v_comment FROM comments WHERE id = p_comment_id AND author_id = auth.uid() AND is_deleted = FALSE FOR UPDATE;
  IF NOT FOUND THEN RETURN jsonb_build_object('success', FALSE, 'error', 'comment_not_found'); END IF;
  UPDATE comments SET is_deleted = TRUE, deleted_at = NOW(), content = '[삭제된 댓글]' WHERE id = p_comment_id;
  RETURN jsonb_build_object('success', TRUE);
END;
$$ LANGUAGE plpgsql SECURITY DEFINER SET search_path = public;
