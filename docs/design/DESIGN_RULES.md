# PulseUp Design Rules

이 문서는 `PulseUp` UI를 구현하거나 확장할 때 반드시 지켜야 하는 화면 규칙을 정리한다. 전제는 `Next.js + Tailwind CSS + shadcn/ui`, 다크 모드 전용이다.
...
- 살아있는 글을 일반 카드처럼 보여주면 `PulseUp`의 핵심 차별점이 사라진다.
